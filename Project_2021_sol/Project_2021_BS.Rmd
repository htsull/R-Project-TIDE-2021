...
ggplot(complete_data, aes(x = duration)) +  geom_density()+
  geom_histogram(fill = 'blue', binwidth = 50, alpha = 1)  + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  labs(title = 'Rerésentation graphique de la durée des films', x = 'Durée', y = 'Effectif') +
...
complete_data$year <- complete_data$year %>% 
  ...
  replace_na(., 2019)
#show unique value of recently create year column
unique(complete_data$year)
```
...
complete_data$decade <- cut(complete_data$year, 
                            breaks = c(seq(min(complete_data$year), max(complete_data$year) + 10, 10)),
                            labels = c("1894 - 1903",
                                       "1904 - 1913",
                                       "1914 - 1923",
                                       "1924 - 1933",
...
```{r}
(j-i)/10
i <- min(complete_data$year)
j <- max(complete_data$year)
a <- seq(i, j+10, 10)
length(a)
```
