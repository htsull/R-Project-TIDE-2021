...
```{r}
genre <- unique(complete_data$genre)
genre_split <- c()
for(i in 1:length(genre)){
  genre_split <- c(genre_split, unlist(str_split(genre[i],",")))
}
genre <- unique(genre_split);genre
```

```{r}
ff<- str_split(complete_data$genre,", ")
y<- intersect(ff,complete_data$genre);y
```



```{r}
avg_vote_par_genre <- function(x){
  d <- complete_data %>%
    filter(grepl(x, genre))
  return(mean(d$avg_vote))
}
notation_film_par_genre <- genre %>%
  sapply(avg_vote_par_genre)
notation_film_par_genre <- as.data.frame(notation_film_par_genre)
names(notation_film_par_genre)<- "avg_vote"
df_avg_vote_par_genre <- data.frame(genre, avg_vote=as.vector(notation_film_par_genre))
couleur <- c(rep(c(1,7),12),1)
couleur2 <- c(1,1,1,7,1,7,1,1,7,7,7,1,1,1,7,1,7,1,1,7,7,1,7,7,7)
ggplot(df_avg_vote_par_genre, aes(x=as.factor(genre), y=avg_vote))+
  geom_point(size=1.5, col=couleur2)+
  geom_segment(aes(x=genre, xend = genre, y=0, yend=avg_vote), col=couleur2)+
  geom_text(aes(label=round(avg_vote, 2), y=avg_vote, vjust=-0.4),col=couleur2)+
  labs(x="genre de film", y="note moyenne sur 10")+
  theme(legend.position = "none", axis.text.x = element_text(angle = 90, hjust = 1, vjust=.4, size = 10, colour = couleur), axis.title = element_text(size = 14))
...
```{r}
# ggplot(complete_data, aes(x = avg_vote_10, color = genre)) +
#   stat_ecdf() +
